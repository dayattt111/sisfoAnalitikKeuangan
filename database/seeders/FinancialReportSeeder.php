    ...
    public function run(): void
    {
        ...
        foreach ($monthsData as $month => $data) {
            ...
            $report1 = FinancialReport::create([
                ...
                'bulan' => $month,
                'tahun' => 2025,
                'judul' => 'Laporan Keuangan ' . $data['name'] . ' 2025',
                'deskripsi' => 'Laporan keuangan bulanan untuk periode ' . $data['name'] . ' 2025. Mencakup semua transaksi pemasukan dan pengeluaran operasional.',
                'periode_mulai' => now()->setYear(2025)->setMonth($month)->startOfMonth()->format('Y-m-d'),
                'periode_akhir' => now()->setYear(2025)->setMonth($month)->endOfMonth()->format('Y-m-d'),
                'status' => 'approved',
            ...
            $report2 = FinancialReport::create([
                ...
                'bulan' => $month,
                'tahun' => 2025,
                'judul' => 'Laporan Keuangan Lapangan ' . $data['name'] . ' 2025',
                'deskripsi' => 'Laporan keuangan dari staff lapangan untuk periode ' . $data['name'] . ' 2025. Data transaksi harian dari operasional lapangan.',
                'periode_mulai' => now()->setYear(2025)->setMonth($month)->startOfMonth()->format('Y-m-d'),
                'periode_akhir' => now()->setYear(2025)->setMonth($month)->endOfMonth()->format('Y-m-d'),
                'status' => $status,
            ...
            if ($month == 3) {
                $report3 = FinancialReport::create([
                    ...
                    'judul' => 'Laporan Keuangan ' . $data['name'] . ' 2024 (Revisi)',
                    'deskripsi' => 'Laporan keuangan bulan ' . $data['name'] . ' 2024 yang perlu diperbaiki dan dilengkapi.',
                    'periode_mulai' => now()->setYear(2024)->setMonth($month)->startOfMonth()->format('Y-m-d'),
                    'periode_akhir' => now()->setYear(2024)->setMonth($month)->endOfMonth()->format('Y-m-d'),
                    'status' => 'rejected',
                    'validated_by' => $manager->id,
