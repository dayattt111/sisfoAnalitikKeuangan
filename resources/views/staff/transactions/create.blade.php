...
<div class="container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-2xl font-bold text-gray-800">Tambah Transaksi Baru</h3>
        ...
        <form action="{{ route('staff.transactions.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="financial_report_id" required
                    ...
                    <option value="{{ $report->id }}" 
                            {{ old('financial_report_id', request('financial_report_id')) == $report->id ? 'selected' : '' }}>
                        Laporan {{ $report->bulan }}/{{ $report->tahun }} ({{ $report->transactions->count() }} transaksi)
                    </option>
                    @endforeach
                </select>
                ...
                <p class="text-yellow-600 text-sm mt-1">
                    <i class="fas fa-exclamation-triangle mr-1"></i>Belum ada laporan pending. 
                    <a href="{{ route('staff.reports.create') }}" class="text-blue-600 underline">Buat laporan baru</a>
                </p>
                @endif
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="jumlah" required min="0" step="1" value="{{ old('jumlah') }}"
                       class="w-full border-gray-300 rounded-lg px-4 py-2 border focus:ring-2 focus:ring-blue-500"
            ...
            <div class="mb-6">
                ...
                <textarea name="keterangan" rows="4"
                          class="w-full border-gray-300 rounded-lg px-4 py-2 border focus:ring-2 focus:ring-blue-500"
                          placeholder="Deskripsi transaksi (opsional)">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
